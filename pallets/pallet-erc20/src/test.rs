...
fn assert_last_event(event: crate::Event<Test>) {
	let last_event = frame_system::Pallet::<Test>::events().pop().expect("Event expected").event;
...
fn mint() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Erc20Token::mint(RawOrigin::Root.into(), account_id, U256::from(100)));
		assert_eq!(Erc20Token::total_supply(), U256::from(100));
		assert_eq!(Erc20Token::balance(account_id), U256::from(100));
		assert_last_event(crate::Event::Mint(account_id, U256::from(100), U256::from(100)));
	});
}
...
fn mint_should_fail_when_no_root_rights() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			Erc20Token::mint(RawOrigin::Signed(account_id).into(), account_id, U256::from(100)),
			BadOrigin
		);
	});
}
...
fn mint_should_fail_when_overflow() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Erc20Token::mint(RawOrigin::Root.into(), account_id, U256::MAX));
...
fn burn() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Erc20Token::burn(RawOrigin::Root.into(), account_id, U256::from(10)));
		assert_eq!(Erc20Token::balance(account_id), U256::from(90));
		assert_eq!(Erc20Token::total_supply(), U256::from(90));
		assert_last_event(crate::Event::Burn(account_id, U256::from(10), U256::from(90)));
	});
}
...
fn burn_should_fail_when_no_root_user() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			Erc20Token::burn(RawOrigin::Signed(account_id).into(), account_id, U256::from(1)),
			BadOrigin
		);
	});
}
...
fn transfer() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Erc20Token::mint(RawOrigin::Root.into(), source_account_id, U256::from(25)));
		assert_ok!(Erc20Token::mint(RawOrigin::Root.into(), dest_account_id, U256::from(36)));
...
fn approve() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Erc20Token::allowance(account_id, spender_account_id), U256::from(15));
		assert_last_event(crate::Event::Approval(account_id, spender_account_id, U256::from(15)));
	});
}
...
fn increase_allowance() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Erc20Token::allowance(account_id, spender_account_id), U256::from(17));
		...
		assert_eq!(Erc20Token::allowance(account_id, spender_account_id), U256::from(19));

		assert_last_event(crate::Event::Approval(account_id, spender_account_id, U256::from(19)));
	});
}
...
fn decrease_allowance() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Erc20Token::allowance(account_id, spender_account_id), U256::from(14));

		assert_last_event(crate::Event::Approval(account_id, spender_account_id, U256::from(14)));
	});
}
...
fn transfer_from() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Erc20Token::mint(RawOrigin::Root.into(), account_id, U256::from(25)));
		assert_ok!(Erc20Token::approve(
			RawOrigin::Signed(account_id).into(),
			spender_account_id,
			U256::from(17)
		...
		assert_eq!(Erc20Token::allowance(account_id, spender_account_id), U256::from(7));

		assert_last_event(crate::Event::Transfer(account_id, dest_account_id, U256::from(10)));
	});
}
